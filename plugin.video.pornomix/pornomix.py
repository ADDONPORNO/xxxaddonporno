...
if addon.getSetting('enable_debug') == 'true':
    enable_debug = True
    xbmc.log('pornomix debug logging enabled')
else:
    enable_debug = False
...
class Main:
    def __init__(self):
        if enable_debug:
            xbmc.log('Initializing pornomix')
        self.pDialog = None
        self.curr_file = ''
        self.urlList = []
        self.extensionList = []
        ...
        if self.currentlist.skill.find('nodownload') == -1:
            if addon.getSetting('download') == 'true':
                ...
                if flv_file == None:
                    ...
                    dialog.ok('pornomix Info', __language__(30053))
    ...
    def parseView(self, url):
        ...
        if ext == 'cfg' or ext == 'list':
            ...
            return -2
        elif ext == 'pornomix' or ext == 'dwnlddevil':
            url = urllib.unquote_plus(url)
            url = url[:len(url) - 11]
           ...
           and self.currentlist.videoCount() == 1:
            ...
            for m in self.currentlist.items:
                ...
                if m_type == 'rss' or m_type == 'search':
                    ...
                    self.addListItem(m_title,
                                     self.currentlist.codeUrl(m, 'pornomix'),
                                     m_icon,
                                     len(self.currentlist.items),
                                     m)
        return result
    ...
    def addListItem(self, title, url, icon, totalItems, lItem):
        ...
        if self.currentlist.getFileExtension(url) == 'pornomix' \
    ...
    def run(self):
        ...
        try:
            ...
            if enable_debug:
                traceback.print_exc(file=sys.stdout)
            dialog = xbmcgui.Dialog()
            dialog.ok('pornomix Error',
                      'Error running pornomix.\n\nReason:\n' + str(e))
